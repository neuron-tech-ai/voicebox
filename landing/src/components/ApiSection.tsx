      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="text-center mb-14">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Every engine you download becomes a REST endpoint on your machine. Build apps, games,
            and voice tools with full programmatic control — no API keys, no rate limits, no
            per-character fees.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 mb-14">
          ...
          <div className="lg:col-span-3 rounded-xl border border-border bg-card/60 backdrop-blur-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-border/60 bg-card/40">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-foreground ml-2">API Reference</span>
              </div>
            ...
            <div className="px-5 py-4 space-y-1">
              {ENDPOINTS.map((ep) => (
                ...
                >
                  ...
                  <code className="text-xs font-mono text-foreground/90">{ep.path}</code>
                  <span className="text-xs text-muted-foreground/60 ml-auto">{ep.label}</span>
                </div>
              ))}
            </div>
            <div className="border-t border-border/60 px-5 py-3 bg-card/40">
              ...
              >
                See the full OpenAPI reference at <code className="font-mono">/docs</code> when
                Voicebox is running →
              </a>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 rounded-xl border border-border bg-card/60 backdrop-blur-sm overflow-hidden flex flex-col">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border/60 bg-card/40">
              ...
              <span className="text-xs font-medium text-foreground">Generate a line</span>
              <span className="ml-auto text-[10px] text-muted-foreground/50 font-mono">curl</span>
            </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-[15px] font-medium text-foreground">{uc.title}</h3>
                </div>
